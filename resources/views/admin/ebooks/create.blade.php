<x-admin-layout title="Crear ebook | {{ config('app.name') }}" :breadcrumbs="[['name' => 'Dashboard','href' => route('admin.dashboard')],['name' => 'Ebooks', 'href' => route('admin.ebooks.index')], ['name' => 'Crear']]">
    <div class="max-w-4xl mx-auto bg-white shadow-sm rounded-lg border border-gray-200 p-6 space-y-4">
        <h1 class="text-xl font-semibold text-gray-900">Nuevo ebook</h1>

        <form method="POST" action="{{ route('admin.ebooks.store') }}" class="space-y-4">
            @csrf
            <div>
                <x-label for="title" value="Título" />
                <x-input id="title" name="title" type="text" class="mt-1 block w-full" value="{{ old('title') }}" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-input id="author" name="author" type="text" class="mt-1 block w-full" value="{{ old('author') }}" />
                    <x-input-error for="author" class="mt-2" />
                </div>
                <div>
                    <x-label for="published_year" value="Año" />
                    <x-input id="published_year" name="published_year" type="number" class="mt-1 block w-full" value="{{ old('published_year') }}" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-input id="isbn" name="isbn" type="text" class="mt-1 block w-full" value="{{ old('isbn') }}" />
                    <x-input-error for="isbn" class="mt-2" />
                </div>
                <div>
                    <x-label for="format" value="Formato (opcional)" />
                    <x-input id="format" name="format" type="text" class="mt-1 block w-full" value="{{ old('format') }}" placeholder="PDF, EPUB, MOBI" />
            ...
            <div>
                ...
                <x-input id="file_path" name="file_path" type="text" class="mt-1 block w-full" value="{{ old('file_path') }}" placeholder="https://..." />
                <x-input-error for="file_path" class="mt-2" />
            </div>

            <div>
                <x-label for="synopsis" value="Sinopsis (opcional)" />
                <textarea id="synopsis" name="synopsis" rows="4" class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('synopsis') }}</textarea>
